...
export class UIManager {
    ...
    drawHUD(ctx, progression, players, gameState, hubStockpile = {}, alienPop = null) {
        ...
        this._drawControlsTab(ctx, players[0], 8 + 128, GAME_H - 80);
        this._drawControlsTab(ctx, players[1], GAME_W - 210 - 128, GAME_H - 80);
    }
    ...
    _drawAlienStatus(ctx, alienPop, progression) {
        ...
        ctx.fillText(`BIOFUEL: ${Math.floor(bH)}%`, 12 + bFoodShake, y + gap * 2);
        ...
        ctx.fillText(`BIOFUEL: ${Math.floor(rH)}%`, GAME_W - 12 + rFoodShake, y + gap * 2);
    ...
    _drawControlsTab(ctx, player, x, y) {
        const isBlue = player.id === 'blue';
        const panelW = 120;
        const panelH = 78;
        ctx.fillStyle = 'rgba(10,10,30,0.8)';
        ctx.fillRect(x - 4, y - 22, panelW, panelH);
        ctx.strokeStyle = isBlue ? '#4488FF66' : '#FF444466';
        ctx.lineWidth = 1;
        ctx.strokeRect(x - 4, y - 22, panelW, panelH);
        ...
        ctx.textAlign = 'left';
        ctx.fillText('CONTROLS', x, y - 8);

        ctx.font = '6px "Press Start 2P"';
        ctx.fillStyle = isBlue ? '#88BBFF' : '#FF8888';
        const controls = isBlue ? [
            'WASD:Move',
            'E:Use',
            'Q:Toss',
            'R:Feed',
    ...
    drawTitleScreen(ctx, time) {
        ...
        ctx.fillText('WASD  E-Interact  Q-Toss  R-Feed', rightX + 14, cardY + 55);
        ctx.fillText('F-Build  T-Process  Tab-Alien', rightX + 14, cardY + 68);
        ctx.fillStyle = '#FF5544';
        ctx.fillText('P2 (Red)', rightX + 14, cardY + 88);
        ctx.fillStyle = '#CC8877';
        ctx.fillText('Arrows  Enter-Interact  .-Toss  O-Feed', rightX + 14, cardY + 103);
    ...
    drawLossScreen(ctx, progression, time, reason) {
        ...
        if (reason === 'aliens') {
            ...
            ctx.fillText('You built the structures in time,', GAME_W / 2, 245);
            ctx.fillText('but not all aliens were rescued to the hub.', GAME_W / 2, 270);
        } else if (reason === 'starvation') {
            ctx.fillText('Without adequate food,', GAME_W / 2, 220);
            ctx.fillText('the alien population perished.', GAME_W / 2, 245);
            ctx.fillText('A tragic end to a hopeful civilization.', GAME_W / 2, 270);
        } else {
            ctx.fillText('The meteors were too powerful.', GAME_W / 2, 220);
