...
function updatePlaying(dt) {
    ...
    // Check for population loss game over
    if (alienPop.blueAliveCount === 0 || alienPop.redAliveCount === 0) {
        state = STATE.LOSS;
        gameLossReason = 'starvation';
        ui.addNotification('A population reached zero!', '#FF4444', 5000);
        audio.stopMusic();
        return;
    }
...
function handlePlayerActions(player, interact, toss, craft, process, cycleL, cycleR, dt) {
    ...
    // Toss item to feed
    const feedToss = (player === player1 ? input.p1Feed() : input.p2Feed());
    if (feedToss && player.currentItem) {
        const isBlue = player.id === 'blue';
        const foodMat = isBlue ? MAT.FIBER : MAT.COAL;
        const targetPos = isBlue ? BLUE_FEEDER_POS : RED_FEEDER_POS;
        if (player.currentItem === foodMat) {
            const mat = player.removeSelectedItem();
            const ti = new TossedItem(mat, player.x, player.y, targetPos.x, targetPos.y);
            tossedItems.push(ti);
            audio.playToss();
            player.showEmote('FEED!');
        } else {
            player.showEmote(`Need ${isBlue ? 'Fiber' : 'Coal'}!`);
            audio.playError();
        }
    }
...
function handleItemLand(item) {
    ...
    // Check if landing on feeders
    if (dist(item.toX, item.toY, BLUE_FEEDER_POS.x, BLUE_FEEDER_POS.y) < 1.0 && item.material === MAT.FIBER) {
        alienPop.feedBlue();
        particles.emitHarvestSparks(sc.x, sc.y, '#55BB55', 8);
        audio.playPickup();
        return;
    }
    if (dist(item.toX, item.toY, RED_FEEDER_POS.x, RED_FEEDER_POS.y) < 1.0 && item.material === MAT.COAL) {
        alienPop.feedRed();
        particles.emitHarvestSparks(sc.x, sc.y, '#FF6633', 8);
        audio.playPickup();
        return;
...
const STRUCTURE_POSITIONS = [
    ...
    { x: ISLANDS.hub.cx - 5, y: ISLANDS.hub.cy + 5, size: 2 },       // Comms Beacon (south-westish)
...
function updateShowerActive(dt) {
    ...
    if (meteorSpawnTimer <= 0) {
        ...
        // Check if landing on solid ground (0 is T.VOID)
        const ftx = Math.floor(tx);
        const fty = Math.floor(ty);
        let solid = false;
        if (world.tiles[fty] && world.tiles[fty][ftx] !== 0) {
            solid = true;
        }
        ...
        activeMeteors.push({
            ...
            maxTimer: fallTime,
            solid: solid,
            trajX: randRange(0.2, 0.6), // falls diagonally from right
            trajY: randRange(0.8, 1.2), // steep drop
        });
    }
    ...
    for (let i = activeMeteors.length - 1; i >= 0; i--) {
        ...
        if (m.timer <= 0) {
            if (m.solid) {
                // IMPACT! â€” explosive
                const iso = isoToScreen(m.x, m.y);
                particles.emitMeteorExplosionWorld(iso.x, iso.y, 30);
                camera.shake(20, 600);
                ...
                // Keep falling through the void off-screen
                if (m.timer < -1500) {
                    activeMeteors.splice(i, 1);
                }
            }
    ...
    if (showerActiveTimer <= 0) {
        ...
        if (showerNum >= TOTAL_SHOWERS) {
            ...
            if (allBuilt && allAliensSafe) {
                ...
                state = STATE.LOSS;
                gameLossReason = !allAliensSafe ? 'aliens' : 'structures';
            }
            audio.stopMusic();
        } else if (actualBuilds < requiredBuilds) {
            state = STATE.LOSS;
            gameLossReason = 'structures';
...
function updateLossScreen() {
    if (input.confirm()) { state = STATE.TITLE; return; }
    ctx.clearRect(0, 0, GAME_W, GAME_H);
    ui.drawLossScreen(ctx, progression, gameTime, gameLossReason);
}
...
function checkLeapZone(player) {
    ...
    for (const zone of LEAP_ZONES) {
        ...
        if (state === STATE.SHOWER_ACTIVE && zone.fromIsland === 'hub') {
            // Block leaping back to home islands during the shower!
            if (player.denyCooldown > 0) return;
            ...
            particles.emitWarningPulse(sc.x, sc.y);
            return;
        }
...
function drawPlayingScene() {
    ...
    ctx.clearRect(0, 0, GAME_W, GAME_H);
    ui.drawSpaceBackground(ctx, gameTime);
...
function drawSceneWithCamera(areaX, areaY, areaW, areaH, cam) {
    ...
    // Add falling meteors to depth sort
    for (const m of activeMeteors) {
        entities.push({
            y: m.y, draw: () => {
                const sc = cam.worldToScreen(m.x, m.y);
                const altitude = (m.timer / m.maxTimer) * 500;
                const curX = sc.x + altitude * m.trajX;
                const curY = sc.y - altitude * m.trajY;
                const z = cam.zoom;
                
                ctx.save();
                
                // Draw extrapolated fading trail
                for (let i = 0; i < 15; i++) {
                    const pastTimer = m.timer + i * 20; // 20ms per segment
                    if (pastTimer > m.maxTimer) continue;
                    const pastAlt = (pastTimer / m.maxTimer) * 500;
                    const px = sc.x + pastAlt * m.trajX;
                    const py = sc.y - pastAlt * m.trajY;
                    
                    const alpha = Math.max(0, 1 - (i / 15));
                    const size = (16 - (i / 15) * 12) * z;
                    const wobble = Math.sin(Date.now() * 0.015 + i) * 3 * z;
                    
                    ctx.fillStyle = `rgba(255, 100, 20, ${alpha * 0.6})`;
                    ctx.fillRect(px - size/2 + wobble, py - size/2, size, size);
                }

                ctx.translate(curX, curY);
                ctx.fillStyle = '#222';
                ctx.fillRect(-12 * z, -12 * z, 24 * z, 24 * z);
                ctx.fillStyle = '#111';
                ctx.fillRect(-8 * z, -8 * z, 16 * z, 16 * z);
                ctx.restore();
            }
        });
    }
    ...
    // Draw feeder text and hunger bar in front of entities
    drawFoodStorageHUD(BLUE_FEEDER_POS, 'blue', cam);
    drawFoodStorageHUD(RED_FEEDER_POS, 'red', cam);
...
function drawFoodStorageHUD(pos, island, cam) {
    const sc = cam.worldToScreen(pos.x, pos.y);
    if (!cam.isOnScreen(sc.x, sc.y)) return;
    const z = cam.zoom;
    const w = 48 * z;
    ...
    ctx.fillText('BIOFUEL', sc.x, by - 10 * z);
    ...
    switch (idx) {
        ...
        case 4: // Hard Reset
            if (typeof localStorage !== 'undefined') {
                localStorage.removeItem('flatEarthrz_tutorialDone');
            }
